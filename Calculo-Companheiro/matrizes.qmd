...
Para indicar os elementos de uma matriz $M_{m\times n}$, usamos a mesma letra que indicamos a matriz, mas em caixa baixa. Ou seja, na matriz $M$ indicada abaixo, o elemento $m_{2,3}$ é o elemento dessa matriz que está na 2ª linha e 3ª coluna.
...
Lê-se: "A matriz $A$, $m$ por $n$, é igual à matriz $B$, $o$ por $p$, se, e somente se, $m$ é igual a $o$ e $n$ é igual a $p$ e, para todo elemento $i$ pertencente ao intervalo de 1 a $m$ e todo $j$ pertencente ao intervalo de 1 a $n$, o elemento $a$ $i,j$ é igual ao elemento $b$ $i,j$.
...
Uma matriz é dita nula se, e somente se, todos os seus elementos são iguais a zero. Alternativamente, A é nula $\rightleftarrows \forall a \in A, \; a = 0$. Como notação de uma matriz nula, utilizamos o próprio algarismo 0, mas com o índice correspondente ao tamanho da matriz.
$$
0_{3\times 2}=
\begin{bmatrix}
    0 & 0  \\
...
Dessa forma, podemos nos perguntar o que aconteceria se tentássemos somar uma matriz $3\times 3$ por uma $2 \times 2$. Pode parecer intuitivo pensar que essa soma poderia ser feita, deixando a linha 3 e coluna 3 da primeira matriz intacta, mas esse pensamento está **Incorreto**: *Podemos apenas somar e subtraír matrizes de mesma ordem.*

## Multiplicação de matrizes por escalar

Chamamos de escalar um número (normalmente real, aqui chamado de $\lambda$) que multiplica um vetor ou matriz. Para multiplicar uma matriz por um escalar, multiplicamos todos seus elementos por ele, um a um, idependente de sua ordem. Isso recebe o nome de produto por escalar
...
*Produto Escalar*: Operação entre duas matrizes (vetores) que nos dá um escalar *Produto POR Escalar*: Operação que acabamos de estudar entre matriz e um escalar que nos dá uma matriz.
