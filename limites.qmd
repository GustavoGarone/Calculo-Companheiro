...
2.1. $\lim\limits_{x\rightarrow a} (f(x) + g(x)) = L_1 + L_2$
2.2. $\lim\limits_{x\rightarrow a} kf(x) = kL_1, k \in \mathbb{R}$ (isso é especialmente útil com $k=-1$, o que permite que 2.1. se apliquee também em subtrações!
2.3. $\lim\limits_{x\rightarrow a} (f(x) \cdot g(x)) = L_1 \cdot L_2$
2.4. $\lim\limits_{x\rightarrow a} \frac{f(x)}{g(x)} = \frac{f(x)}{g(x)}, L_2 \neq 0$
